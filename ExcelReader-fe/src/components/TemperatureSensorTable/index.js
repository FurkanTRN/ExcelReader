...
const SensorSummaryTable = ({ sensorData }) => {
  const calculateSensorStats = (sensorData) => {
    // "Sıcaklık" ile başlayan değerleri filtreler
    const filteredData = sensorData.filter(entry => entry.name.startsWith("Sıcaklık"));

    // Filtrelenmiş verileri cihaz adına göre gruplar
          ...
          <TableBody>
            {stats.map((row) => (
              <StyledTableRow key={row.deviceName}>
                ...
                <StyledTableCell align="right">{row.min.toFixed(2)} °C</StyledTableCell>
                <StyledTableCell align="right">{row.max.toFixed(2)} °C</StyledTableCell>
                <StyledTableCell align="right">{row.avg.toFixed(2)} °C</StyledTableCell>
              </StyledTableRow>
            ))}
          </TableBody>
        </Table>
      ...
      <TableContainer component={Paper} sx={{ boxShadow: 'none', border: '1px solid rgba(224, 224, 224, 1)', marginTop: '16px', maxWidth: '400px' }}>
        <Table>
          ...
          <TableBody>
            ...
            <StyledTableRow>
              <StyledTableCell component="th" scope="row">Average Temperature</StyledTableCell>
              <StyledTableCell align="right">{avgOfAvgs.toFixed(2)} °C</StyledTableCell>
            </StyledTableRow>
          </TableBody>
        </Table>
      </TableContainer>
...
const TemparatureSensorTable = (props) => (
  <ThemeProvider theme={theme}>
    <SensorSummaryTable {...props} />
  </ThemeProvider>
);

export default TemparatureSensorTable;
